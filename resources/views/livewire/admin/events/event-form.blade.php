<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">{{ $eventId ? 'Edit' : 'Create' }} Event</h1>
        <a href="{{ route('admin.events.index') }}" class="px-4 py-2 text-sm font-medium text-neutral-700 dark:text-neutral-300 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-700">Back to List</a>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select wire:model="category_id" class="w-full px-3 py-2.5 border border-neutral-300 dark:border-neutral-700 rounded-lg text-sm bg-white dark:bg-neutral-800 dark:text-white">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Event Type *</label>
                    <select wire:model.live="event_type" class="w-full px-3 py-2.5 border border-neutral-300 dark:border-neutral-700 rounded-lg text-sm bg-white dark:bg-neutral-800 dark:text-white">
                        <option value="offline">Offline</option>
                        <option value="online">Online</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Event Date & Time</label>
                    <input type="datetime-local" wire:model="event_date" class="w-full px-3 py-2.5 border border-neutral-300 dark:border-neutral-700 rounded-lg text-sm bg-white dark:bg-neutral-800 dark:text-white">
                    @error('event_date') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            {{-- Online Link (shown when online) --}}
            @if ($event_type === 'online')
                <div>
                    <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Online Link *</label>
                    <input type="url" wire:model="online_link" placeholder="https://meet.google.com/..." class="w-full px-3 py-2.5 border border-neutral-300 dark:border-neutral-700 rounded-lg text-sm bg-white dark:bg-neutral-800 dark:text-white">
                    @error('online_link') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
            @endif

            {{-- Venue (shown when offline) --}}
            @if ($event_type === 'offline')
                <div>
                    <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Venue *</label>
                    <textarea wire:model="venue" rows="2" maxlength="500" placeholder="Enter the venue address..." class="w-full px-3 py-2.5 border border-neutral-300 dark:border-neutral-700 rounded-lg text-sm bg-white dark:bg-neutral-800 dark:text-white resize-none"></textarea>
            ...
            <div wire:ignore>
                ...
                <div
                    x-data="{ editor: null }"
                    x-init="
                        ClassicEditor.create($refs.editor, { toolbar: ['heading', '|', 'bold', 'italic', 'link', 'bulletedList', 'numberedList', '|', 'blockQuote', 'insertTable', 'mediaEmbed', 'undo', 'redo'] })
                            .then(e => {
                                editor = e;
            ...
            <div>
                ...
                @if ($existing_thumbnail && !$thumbnail_image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($existing_thumbnail) }}" class="w-24 h-16 rounded-lg object-cover border">
                    </div>
                @endif
                @if ($thumbnail_image)
                    <div class="mb-2">
                        <img src="{{ $thumbnail_image->temporaryUrl() }}" class="w-24 h-16 rounded-lg object-cover border">
                    </div>
                @endif
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.events.index') }}" class="px-5 py-2.5 text-sm font-medium text-neutral-700 dark:text-neutral-300 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg">Cancel</a>
            <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-[#464d79] rounded-lg hover:bg-[#3a4166] transition-colors">{{ $eventId ? 'Update Event' : 'Create Event' }}</button>
        </div>
    </form>
</div>
