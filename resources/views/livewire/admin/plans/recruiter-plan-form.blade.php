<div class="max-w-3xl">
    <h1 class="text-2xl font-bold text-neutral-900 dark:text-white mb-6">{{ $plan ? 'Edit' : 'Create' }} Recruiter Plan</h1>
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-1">Recruiter Type</label>
                    <select wire:model="recruiter_type" class="w-full h-10 px-3 border rounded-lg text-sm" {{ $plan ? 'disabled' : '' }}>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6 space-y-4">
            ...
            <div class="border border-neutral-200 dark:border-neutral-700 rounded-lg p-4 space-y-3">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                        <label class="text-xs text-neutral-500">Duration (months)</label>
                ...
                <div class="flex items-center gap-4">
                    <label class="flex items-center gap-2 text-sm"><input type="checkbox" wire:model="options.{{ $i }}.is_unlimited_postings" class="rounded"> Unlimited postings</label>
                    @if(!($options[$i]['is_unlimited_postings'] ?? false))
                    <div>
                        <label class="text-xs text-neutral-500">Postings/month</label>
                        <input type="number" wire:model="options.{{ $i }}.job_postings_per_month" class="w-20 h-8 px-2 border rounded text-sm" min="1" />
                    </div>
                    @endif
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.plans.recruiter.index') }}" wire:navigate class="h-10 px-6 inline-flex items-center rounded-lg text-sm font-medium bg-neutral-100 text-neutral-700 hover:bg-neutral-200">Cancel</a>
        </div>
    </form>
</div>
