<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-neutral-500 mt-1">Platform-wide job application oversight.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 text-center">
            ...
            <p class="text-xs text-neutral-500 mt-1">Today</p>
        </div>
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 text-center">
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 text-center">
            <p class="text-2xl font-bold text-neutral-500">{{ $stats['binned'] }}</p>
            <p class="text-xs text-neutral-500 mt-1">In Bin</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search applicant, job, institution..." class="h-10 px-3 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm focus:outline-none focus:border-[#464d79] focus:ring-2 focus:ring-[#464d79]/20"/>
            <select wire:model.live="status" class="h-10 px-3 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm focus:outline-none focus:border-[#464d79]">
                <option value="">All Statuses</option>
                @foreach($statuses as $s)
                    <option value="{{ $s->value }}">{{ $s->label() }}</option>
                @endforeach
            </select>
            <select wire:model.live="ranking" class="h-10 px-3 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm focus:outline-none focus:border-[#464d79]">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-700">
                ...
                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-900/50">
                    ...
                    <td class="px-4 py-3 text-neutral-500">{{ $app->job?->institution_name }}</td>
                    ...
                    <td class="px-4 py-3 text-right">
                        <a href="{{ route('admin.applications.show', $app) }}" wire:navigate class="text-[#464d79] hover:underline text-xs font-medium">View</a>
                    </td>
                </tr>
                @empty
                <tr><td colspan="7" class="px-4 py-8 text-center text-neutral-500">No applications found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
