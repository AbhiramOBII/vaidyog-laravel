<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.news-categories.create') }}" class="px-4 py-2 text-sm font-medium text-white bg-[#464d79] rounded-lg hover:bg-[#3a4166] transition-colors">+ New Category</a>
        @endif
    </div>
                    ...
                    <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div class="w-8 h-8 rounded bg-neutral-100 dark:bg-neutral-700 flex items-center justify-center text-neutral-400 text-xs">N</div>
                                @endif
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <a href="{{ route('admin.news-categories.edit', $cat->id) }}" class="px-2 py-1 text-xs font-medium text-[#464d79] hover:bg-[#464d79]/10 rounded transition-colors">Edit</a>
                                @endif
                                @if(auth('admin')->user()->hasPermission('news.delete'))
                                <button wire:click="delete({{ $cat->id }})" wire:confirm="Delete this category? All news under it will also be deleted." class="px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors">Delete</button>
                                @endif
                            </div>
                        </td>
                    </tr>
