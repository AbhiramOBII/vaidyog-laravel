<div class="min-h-screen grid grid-cols-1 lg:grid-cols-12">
    ...
    {{-- RIGHT PANEL (5 cols) — Login Form --}}
    <div class="lg:col-span-5 bg-white flex items-center justify-center p-6 sm:p-8 lg:p-10 min-h-screen">
        <div class="w-full max-w-md">
            ...
            {{-- Form header --}}
            <div class="mb-6">
                <h2 class="text-2xl font-bold tracking-tight text-neutral-900 mb-1">Welcome back</h2>
                <p class="text-sm text-neutral-500">Sign in to manage your recruitment dashboard.</p>
            </div>

            {{-- Method Toggle --}}
            <div class="flex items-center gap-1 p-1 bg-neutral-100 rounded-lg mb-6">
                <button wire:click="$set('loginMethod', 'email')" class="flex-1 py-2 text-xs font-medium rounded-md transition-colors {{ $loginMethod === 'email' ? 'bg-white text-[#464d79] shadow-sm' : 'text-neutral-500 hover:text-neutral-700' }}">
                    Email & Password
                </button>
                <button wire:click="$set('loginMethod', 'phone')" class="flex-1 py-2 text-xs font-medium rounded-md transition-colors {{ $loginMethod === 'phone' ? 'bg-white text-[#464d79] shadow-sm' : 'text-neutral-500 hover:text-neutral-700' }}">
                    Phone OTP
                </button>
            </div>
            ...
            @if ($loginMethod === 'email')
                <form wire:submit="loginWithEmail" novalidate>
                    <div class="mb-5">
                    ...
                    <div class="mb-6" x-data="{ show: false }">
                        ...
                        <div class="relative">
                            <input wire:model="password" :type="show ? 'text' : 'password'" placeholder="••••••••" class="w-full h-11 px-4 pr-12 bg-neutral-50 border border-neutral-200 rounded-lg text-sm placeholder-neutral-400 focus:outline-none focus:border-[#464d79] focus:ring-2 focus:ring-[#464d79]/20 transition-all @error('password') border-red-400 @enderror"/>
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="w-full h-12 bg-[#464d79] hover:bg-[#3a4169] active:bg-[#2f3357] text-white font-semibold rounded-lg text-sm flex items-center justify-center gap-2 transition-colors shadow-sm hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed">
                        <span wire:loading wire:target="loginWithEmail" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
                        <span wire:loading.remove wire:target="loginWithEmail" class="flex items-center gap-2">
            ...
            @if ($loginMethod === 'phone')
                @if (!$otpSent)
                    <form wire:submit="sendPhoneOtp" novalidate>
                        ...
                        <button type="submit" wire:loading.attr="disabled" class="w-full h-12 bg-[#464d79] hover:bg-[#3a4169] active:bg-[#2f3357] text-white font-semibold rounded-lg text-sm flex items-center justify-center gap-2 transition-colors shadow-sm hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed">
                            <span wire:loading wire:target="sendPhoneOtp" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
                            <span wire:loading.remove wire:target="sendPhoneOtp">Send OTP</span>
                        </button>
                    </form>
                @else
                    <div class="mb-4">
                        <p class="text-sm text-neutral-600">OTP sent to <strong class="text-neutral-900">+91 {{ $phone }}</strong></p>
                    </div>
                    <form wire:submit="verifyPhoneOtp" novalidate>
                        <div class="mb-6">
                            <input wire:model="otp" type="text" maxlength="6" placeholder="000000" class="w-full h-14 px-4 bg-neutral-50 border border-neutral-200 rounded-lg text-center text-2xl font-mono font-bold tracking-[0.5em] focus:outline-none focus:border-[#464d79] focus:ring-2 focus:ring-[#464d79]/20"/>
                            @error('otp') <p class="text-xs text-red-600 mt-1.5 text-center">{{ $message }}</p> @enderror
                        </div>

                        <button type="submit" wire:loading.attr="disabled" class="w-full h-12 bg-[#464d79] hover:bg-[#3a4169] active:bg-[#2f3357] text-white font-semibold rounded-lg text-sm flex items-center justify-center gap-2 transition-colors shadow-sm hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed">
                            <span wire:loading wire:target="verifyPhoneOtp" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
                            <span wire:loading.remove wire:target="verifyPhoneOtp" class="flex items-center gap-2">
                                Verify & Login
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3 10a.75.75 0 01.75-.75h10.638L11.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 11-1.04-1.08l3.158-2.96H3.75A.75.75 0 013 10z" clip-rule="evenodd"/></svg>
                            </span>
                        </button>
                    ...
                    <div class="mt-4 text-center" x-data="{ countdown: @entangle('resendCooldown'), interval: null }" x-init="interval = setInterval(() => { if(countdown > 0) countdown--; }, 1000); $watch('countdown', v => { if(v <= 0) clearInterval(interval) })">
                        <template x-if="countdown > 0">
                            <p class="text-sm text-neutral-500">Resend in <span x-text="countdown" class="font-medium text-[#464d79]"></span>s</p>
                        </template>
                        <template x-if="countdown <= 0">
                            <button wire:click="resendOtp" class="text-sm font-medium text-[#464d79] hover:underline">Resend OTP</button>
                        </template>
                    </div>

                    <button wire:click="$set('otpSent', false)" class="w-full mt-3 text-sm text-neutral-500 hover:text-neutral-700 text-center">← Change number</button>
                @endif
            @endif

            {{-- Google Login --}}
            <div class="mt-5 pt-5 border-t border-neutral-100">
                <a href="/auth/google/recruiter/redirect" class="w-full h-11 inline-flex items-center justify-center gap-3 border border-neutral-200 rounded-lg text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-colors">
            ...
            <p class="mt-5 text-center text-sm text-neutral-500">
                Don't have an account? <a href="{{ route('recruiter.register') }}" class="font-medium text-[#464d79] hover:underline">Register here</a>
            </p>
        </div>
    </div>
</div>
