...
#[Layout('components.layouts.admin', ['pageTitle' => 'Event Category'])]
class EventCategoryForm extends Component
{
    ...
    {
        if ($category) {
            $cat = EventCategory::findOrFail($category);
    ...
    public function save(): void
    {
        $rules = [
            ...
            'slug' => 'required|max:200|unique:event_categories,slug' . ($this->categoryId ? ",{$this->categoryId}" : ''),
        ...
        if ($this->thumbnail_image) {
            $data['thumbnail_image'] = $this->thumbnail_image->store('event-categories', 'public');
        }

        if ($this->categoryId) {
            EventCategory::findOrFail($this->categoryId)->update($data);
        ...
        $this->redirect(route('admin.event-categories.index'), navigate: true);
    }

    public function render()
    {
        $parentCategories = EventCategory::parents()
            ->when($this->categoryId, fn($q) => $q->where('id', '!=', $this->categoryId))
            ->active()
            ->pluck('title', 'id');

        return view('livewire.admin.events.event-category-form', [
