...
#[Layout('components.layouts.admin', ['pageTitle' => 'News Category'])]
class NewsCategoryForm extends Component
{
    ...
    {
        if ($category) {
            $cat = NewsCategory::findOrFail($category);
    ...
    public function save(): void
    {
        $rules = [
            ...
            'slug' => 'required|max:200|unique:news_categories,slug' . ($this->categoryId ? ",{$this->categoryId}" : ''),
            'parent_id' => 'nullable|exists:news_categories,id',
            'short_description' => 'nullable|max:500',
            'full_description' => 'nullable',
            'status' => 'required|in:active,inactive',
            'thumbnail_image' => $this->categoryId ? 'nullable|image|max:2048' : 'nullable|image|max:2048',
        ...
        if ($this->thumbnail_image) {
            $data['thumbnail_image'] = $this->thumbnail_image->store('news-categories', 'public');
        }
    ...
    {
        $parentCategories = NewsCategory::parents()
            ->when($this->categoryId, fn($q) => $q->where('id', '!=', $this->categoryId))
            ->active()
            ->pluck('title', 'id');

        return view('livewire.admin.news.news-category-form', [
